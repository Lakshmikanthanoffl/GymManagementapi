        ...
        try
        {
            ...
            if (isValid)
            {
                ...
                if (role != null)
                {
                    ...
                    // ✅ Send payment notification to zyct.official@gmail.com
                    var emailSubject = $"Payment Received from {role.UserName}";
                    var emailBody = $@"
                    Hello CEO,
                    ...
                    Details:
                    - User: {role.UserName} ({role.UserEmail})
                    - Gym: {role.GymName}
                    - Plan: {role.PlanName}
                    - Subscription Period: {role.SubscriptionPeriod}
                    - Amount Paid: {role.AmountPaid}
                    - Paid Date (UTC): {role.PaidDate?.ToString("yyyy-MM-dd HH:mm:ss")}
